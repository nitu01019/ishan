...
export default function FAQsPage() {
  ...
  function updateField(field: keyof FAQFormData, value: string) {
    setForm((prev) => ({ ...prev, [field]: value }));
  }

  if (loading) {
    return <p className="text-text-secondary">Loading FAQs...</p>;
  }
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="md:col-span-3">
                <label className="block text-text-secondary text-sm mb-1">Question</label>
                <input
                  type="text"
                  value={form.question}
            ...
            <div>
              <label className="block text-text-secondary text-sm mb-1">Answer</label>
              <textarea
                value={form.answer}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-700">
                ...
                <th className="text-left text-text-secondary text-sm font-medium p-4">Question</th>
                <th className="text-left text-text-secondary text-sm font-medium p-4">Answer</th>
            ...
            <tbody>
              {faqs.length === 0 ? (
                ...
                faqs.map((faq) => (
                  <tr key={faq.id} className="border-b border-gray-700/50 hover:bg-bg-card-alt/50 transition-colors">
                    <td className="p-4 text-text-secondary text-sm">{faq.order}</td>
                    <td className="p-4 text-white text-sm">{faq.question}</td>
