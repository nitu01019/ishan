...
export default function RadialOrbitalTimeline({
  ...
  const toggleItem = (id: number) => {
    setExpandedItems((prev) => {
      ...
      Object.keys(prev).forEach((key) => {
        newState[parseInt(key)] = parseInt(key) === id ? !prev[parseInt(key)] : false;
      });
      if (!(id in prev)) newState[id] = true;
      else newState[id] = !prev[id];
  ...
  const getStatusStyles = (status: TimelineItem["status"]): string => {
    switch (status) {
      case "completed": return "text-black bg-accent-green border-accent-green";
            ...
            return (
              ...
              >
                ...
                <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${
                  isExpanded ? "bg-accent-green text-black border-accent-green shadow-lg shadow-accent-green/30 scale-150"
                  : isRelated ? "bg-accent-green/50 text-black border-accent-green animate-pulse"
                  : "bg-[#0B1120] text-white border-white/40"
                }`}>
                  <Icon size={16} />
                </div>
                ...
                {/* Expanded card */}
                {isExpanded && (
                  <Card className="absolute top-20 left-1/2 -translate-x-1/2 w-64 bg-[#111827]/95 backdrop-blur-lg border-white/20 shadow-xl shadow-accent-green/10 overflow-visible">
                    ...
                    <CardContent className="text-xs text-white/80">
                      ...
                      <div className="mt-4 pt-3 border-t border-white/10">
                        ...
                        <div className="w-full h-1 bg-white/10 rounded-full overflow-hidden">
                          <div className="h-full bg-gradient-to-r from-accent-green to-accent-cyan" style={{ width: `${item.energy}%` }} />
                        </div>
                      </div>
